...
int main(int argc, char* argv[]) {
	...
	//	return 1;
	//}
	int webserver_port = 0;
	string webserver_addr;

	if(argc > 1){
	flic_hostname = argv[1];
	flic_port = atoi(argv[2]);
	webserver_addr = argv[3];
	...
	// use defaults
	//localhost 5551 127.0.0.1 5553
	flic_hostname = "localhost";
	//flic_hostname = hostname;
	flic_port = 5551;
	...
	while (true) {
		...
		std::cout << "using flic_hostname " << flic_hostname << std::endl;
		std::cout << "using webserver_port " << webserver_port << std::endl;
		std::cout << std::endl;
